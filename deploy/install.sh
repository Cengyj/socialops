#!/bin/bash
#
# SocialOps Installation Script
# SocialOps 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Wei-Shaw/socialops/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Wei-Shaw/socialops"
INSTALL_DIR="/opt/socialops"
SERVICE_NAME="socialops"
SERVICE_USER="socialops"
CONFIG_DIR="/etc/socialops"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="SocialOps 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 SocialOps 服务："
    ...
    ["not_installed"]="SocialOps 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 SocialOps。"
    ...
    ["uninstall_complete"]="SocialOps 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 SocialOps"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 SocialOps"
    ...
    ["server_config_desc"]="配置 SocialOps 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="SocialOps Installation Script"
    ...
    # Completion
    ["install_complete"]="SocialOps installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start SocialOps service:"
    ...
    ["not_installed"]="SocialOps is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove SocialOps from your system."
    ...
    ["uninstall_complete"]="SocialOps has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install SocialOps"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove SocialOps"
    ...
    ["server_config_desc"]="Configure SocialOps server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/socialops" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/socialops" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="socialops_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/socialops" "$INSTALL_DIR/socialops"
    chmod +x "$INSTALL_DIR/socialops"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/socialops"
}
...
User=socialops
Group=socialops
WorkingDirectory=/opt/socialops
ExecStart=/opt/socialops/socialops
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=socialops
...
start_service() {
    ...
    if systemctl start socialops; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u socialops -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable socialops 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status socialops"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u socialops -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart socialops"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop socialops"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if SocialOps is installed
    if [ ! -f "$INSTALL_DIR/socialops" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/socialops" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet socialops; then
        print_info "$(msg 'stopping_service')"
        systemctl stop socialops
    fi

    # Backup current binary
    cp "$INSTALL_DIR/socialops" "$INSTALL_DIR/socialops.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/socialops.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/socialops"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start socialops

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: SocialOps must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/socialops" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="socialops.backup.${current_version}"
        else
            backup_name="socialops.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/socialops" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop socialops 2>/dev/null || true
    systemctl disable socialops 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/socialops.service
    systemctl daemon-reload

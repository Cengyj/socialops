...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	systemOperationLockService := service.ProvideSystemOperationLockService(configConfig)
	...
	adminUsageHandler := admin.NewUsageHandler(usageService, userService, apiKeyService, usageCleanupService)
	...
	socialAccountAdminHandler := admin.NewSocialAccountAdminHandler(socialAccountService, socialIPService, socialBillingService, socialTaskExecutor)
	socialIPChecker := service.NewSocialIPChecker(client)
	proxyHandler := admin.NewProxyHandler(socialIPService, socialIPChecker)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, adminRedeemHandler, promoHandler, settingHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, adminAPIKeyHandler, paymentHandler, affiliateHandler, socialAccountAdminHandler, proxyHandler)
	...
	socialAccountHandler := handler.NewSocialAccountHandler(socialAccountService, socialIPService, socialBillingService, socialTaskExecutor)
	planService := service.NewPlanService(client)
	planHandler := handler.NewPlanHandler(planService)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, socialAccountHandler, planHandler)
	...
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(configConfig)

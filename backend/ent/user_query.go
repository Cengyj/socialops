...
type UserQuery struct {
	...
	withSocialAccounts        *SocialAccountQuery
	withSocialTaskLogs        *SocialTaskLogQuery
	withSocialIps             *SocialIPQuery
...
// QuerySocialAccounts chains the current query on the "social_accounts" edge.
func (_q *UserQuery) QuerySocialAccounts() *SocialAccountQuery {
...
// QuerySocialTaskLogs chains the current query on the "social_task_logs" edge.
func (_q *UserQuery) QuerySocialTaskLogs() *SocialTaskLogQuery {
...
// QuerySocialIps chains the current query on the "social_ips" edge.
func (_q *UserQuery) QuerySocialIps() *SocialIPQuery {
	query := (&SocialIPClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSocialAccounts:        _q.withSocialAccounts.Clone(),
		withSocialTaskLogs:        _q.withSocialTaskLogs.Clone(),
		withSocialIps:             _q.withSocialIps.Clone(),
...
// WithSocialAccounts tells the query-builder to eager-load the nodes that are connected to
// the "social_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSocialAccounts(opts ...func(*SocialAccountQuery)) *UserQuery {
	query := (&SocialAccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSocialAccounts = query
	return _q
}

// WithSocialTaskLogs tells the query-builder to eager-load the nodes that are connected to
// the "social_task_logs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSocialTaskLogs(opts ...func(*SocialTaskLogQuery)) *UserQuery {
	query := (&SocialTaskLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSocialTaskLogs = query
	return _q
}

// WithSocialIps tells the query-builder to eager-load the nodes that are connected to
// the "social_ips" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSocialIps(opts ...func(*SocialIPQuery)) *UserQuery {
	query := (&SocialIPClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSocialIps = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [16]bool{
			...
			_q.withSocialAccounts != nil,
			_q.withSocialTaskLogs != nil,
			_q.withSocialIps != nil,
	...
	if query := _q.withSocialAccounts; query != nil {
		if err := _q.loadSocialAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.SocialAccounts = []*SocialAccount{} },
			func(n *User, e *SocialAccount) { n.Edges.SocialAccounts = append(n.Edges.SocialAccounts, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSocialTaskLogs; query != nil {
		if err := _q.loadSocialTaskLogs(ctx, query, nodes,
			func(n *User) { n.Edges.SocialTaskLogs = []*SocialTaskLog{} },
			func(n *User, e *SocialTaskLog) { n.Edges.SocialTaskLogs = append(n.Edges.SocialTaskLogs, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSocialIps; query != nil {
		if err := _q.loadSocialIps(ctx, query, nodes,
			func(n *User) { n.Edges.SocialIps = []*SocialIP{} },
			func(n *User, e *SocialIP) { n.Edges.SocialIps = append(n.Edges.SocialIps, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSocialAccounts(ctx context.Context, query *SocialAccountQuery, nodes []*User, init func(*User), assign func(*User, *SocialAccount)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(socialaccount.FieldAssignedUserID)
	}
	query.Where(predicate.SocialAccount(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SocialAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.AssignedUserID
		if fk == nil {
			return fmt.Errorf(`foreign-key "assigned_user_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "assigned_user_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadSocialTaskLogs(ctx context.Context, query *SocialTaskLogQuery, nodes []*User, init func(*User), assign func(*User, *SocialTaskLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(socialtasklog.FieldUserID)
	}
	query.Where(predicate.SocialTaskLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SocialTaskLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadSocialIps(ctx context.Context, query *SocialIPQuery, nodes []*User, init func(*User), assign func(*User, *SocialIP)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(socialip.FieldUserID)
	}
	query.Where(predicate.SocialIP(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SocialIpsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

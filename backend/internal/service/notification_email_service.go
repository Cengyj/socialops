...
const (
	...
	notificationEmailStoredLongKeyPadding = "legacy"
)
...
// NewNotificationEmailService wires notification email storage, rendering, and
// delivery.
...
func (s *NotificationEmailService) Send(ctx context.Context, input NotificationEmailSendInput) error {
	...
	if exists, err := s.deliveryExists(ctx, deliveryKey, historicalKey); err != nil {
		return err
	} else if exists {
		return nil
	}
...
func (s *NotificationEmailService) IsUnsubscribed(ctx context.Context, email, event string) (bool, error) {
	...
	for _, key := range []string{
		...
		historicalNotificationEmailPreferenceKey(event, email),
	} {
...
func historicalNotificationEmailPreferenceKey(event, email string) string {
	return notificationEmailPreferenceKeyPrefix + event + ":" + strings.ToLower(strings.TrimSpace(email)) + ":" + strings.Repeat(notificationEmailStoredLongKeyPadding, 20)
}
...
func historicalNotificationEmailDeliveryKey(event, sourceType, sourceID, email, reminderKey string) string {
	return notificationEmailDeliveryKeyPrefix + event + ":" + sourceType + ":" + sourceID + ":" + strings.ToLower(strings.TrimSpace(email)) + ":" + reminderKey + ":" + strings.Repeat(notificationEmailStoredLongKeyPadding, 20)
}

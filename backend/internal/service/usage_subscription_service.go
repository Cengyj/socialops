...
// UsageLog represents SocialOps social task activity and charge records.
...
type dashboardUsageTrendProvider interface {
	GetUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string) ([]usagestats.TrendDataPoint, error)
}
...
func (s *UsageService) List(ctx context.Context, params pagination.PaginationParams, filters usagestats.UsageLogFilters) ([]UsageLog, *pagination.PaginationResult, error) {
	normalizedFilters, err := normalizeUsageLogFilters(filters)
	if err != nil {
		return nil, nil, err
	}
	if repo, ok := s.repo.(usageLogLister); ok {
		return repo.ListWithFilters(ctx, params, normalizedFilters)
	}
...
func (s *UsageService) Stats(ctx context.Context, filters usagestats.UsageLogFilters) (*usagestats.UsageStats, error) {
	...
	if repo, ok := s.repo.(usageLogLister); ok {
		return repo.GetStatsWithFilters(ctx, normalizedFilters)
	}
	return nil, ErrUsageServiceUnavailable
}

func normalizeUsageLogFilters(filters usagestats.UsageLogFilters) (usagestats.UsageLogFilters, error) {
	operation := strings.ToLower(strings.TrimSpace(filters.Operation))
	if operation != "" {
		normalized, ok := NormalizeSocialTaskAction(operation)
		if !ok {
			return filters, ErrSocialTaskUnsupportedAction
		}
		filters.Operation = normalized
	} else {
		filters.Operation = ""
	}

	status := strings.ToLower(strings.TrimSpace(filters.Status))
	if status != "" && status != SocialTaskLogStatusSuccess && status != SocialTaskLogStatusFailed {
		return filters, infraerrors.BadRequest("USAGE_STATUS_INVALID", "usage status must be success or failed")
	}
...
func (s *DashboardService) GetUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string) ([]usagestats.TrendDataPoint, error) {
	if s != nil {
		if repo, ok := s.repo.(dashboardUsageTrendProvider); ok {
			return repo.GetUsageTrend(ctx, startTime, endTime, granularity)
		}
	}
	return nil, ErrUsageServiceUnavailable
}

...
func TestUsageCleanupServiceCreateTaskSanitizeFilters(t *testing.T) {
	...
	require.NotNil(t, task.Filters.Operation)
	require.Equal(t, "follow", *task.Filters.Operation)
	require.NotNil(t, task.Filters.Status)
	require.Equal(t, "success", *task.Filters.Status)
	require.Equal(t, int64(9), task.CreatedBy)
}

func TestSanitizeUsageCleanupFiltersTrimsSocialFields(t *testing.T) {
	operation := " message "
	status := " pending "
	filters := UsageCleanupFilters{
		Operation: &operation,
	...
	require.NotNil(t, filters.Operation)
	require.Equal(t, "message", *filters.Operation)
	require.NotNil(t, filters.Status)
	require.Equal(t, "pending", *filters.Status)
}

func TestSanitizeUsageCleanupFiltersEmptySocialFields(t *testing.T) {
	operation := "   "
	status := ""
	filters := UsageCleanupFilters{
		Operation: &operation,
	...
	require.Nil(t, filters.Operation)
	require.Nil(t, filters.Status)
}

func TestDescribeUsageCleanupFiltersIncludesOperation(t *testing.T) {
	start := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)
	operation := "follow"
	...
	require.Contains(t, desc, "operation=follow")
}
...
func TestUsageCleanupServiceDefaultsAndLifecycle(t *testing.T) {
	...
	svcFallback := NewUsageCleanupService(repo, nil, nil, cfgFallback)
...
func TestSanitizeUsageCleanupFiltersOperationEmpty(t *testing.T) {
	operation := "   "
	status := " "
	apiKeyID := int64(-5)
	accountID := int64(-1)
...
func TestDescribeUsageCleanupFiltersAllFields(t *testing.T) {
	...
	require.Equal(t, "start=2024-02-01T10:00:00Z end=2024-02-01T12:00:00Z user_id=1 api_key_id=2 account_id=3 group_id=4 operation=follow status=success", desc)
}

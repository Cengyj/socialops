...
func TestSettingService_GetAuthSourceDefaultSettings_ParsesValuesAndDefaults(t *testing.T) {
	repo := &authSourceDefaultsRepoStub{
		values: map[string]string{
			...
			SettingKeyAuthSourceDefaultEmailSubscriptions:      `[{"plan_id":101,"group_id":11,"validity_days":30}]`,
	...
	require.Equal(t, []DefaultSubscriptionSetting{{PlanID: 101, GroupID: 11, ValidityDays: 30}}, got.Email.Subscriptions)
...
func TestSettingService_UpdateAuthSourceDefaultSettings_PersistsAllKeys(t *testing.T) {
	...
	err := svc.UpdateAuthSourceDefaultSettings(context.Background(), &AuthSourceDefaultSettings{
		Email: ProviderDefaultGrantSettings{
			Balance:          1.25,
			Concurrency:      3,
			Subscriptions:    []DefaultSubscriptionSetting{{PlanID: 201, GroupID: 21, ValidityDays: 14}},
			GrantOnSignup:    false,
		...
		LinuxDo: ProviderDefaultGrantSettings{
			Balance:          2,
			Concurrency:      4,
			Subscriptions:    []DefaultSubscriptionSetting{{PlanID: 202, GroupID: 22, ValidityDays: 30}},
			GrantOnSignup:    true,
		...
		OIDC: ProviderDefaultGrantSettings{
			Balance:          3,
			Concurrency:      5,
			Subscriptions:    []DefaultSubscriptionSetting{{PlanID: 203, GroupID: 23, ValidityDays: 60}},
			GrantOnSignup:    true,
			GrantOnFirstBind: true,
		},
		WeChat: ProviderDefaultGrantSettings{
			Balance:          4,
			Concurrency:      6,
			Subscriptions:    []DefaultSubscriptionSetting{{PlanID: 204, GroupID: 24, ValidityDays: 90}},
			GrantOnSignup:    false,
			GrantOnFirstBind: false,
		},

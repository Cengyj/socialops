...
func TestAPIKeyAuthIPRestrictionDoesNotTrustForwardedClientIPByDefault(t *testing.T) {
	...
	router.Use(func(c *gin.Context) {
		c.Next()
		if v, ok := c.Get(service.ClientAccessLimitedKey); ok {
			markedBusinessLimited, _ = v.(bool)
		}
		if v, ok := c.Get(service.ClientAccessLimitedReasonKey); ok {
	...
	require.Equal(t, service.ClientAccessLimitedReasonIPRestriction, businessLimitedReason)
}
...
func TestAPIKeyAuthAllowsUngroupedStandardModeKeyWithBalance(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func (r *stubUserSubscriptionRepo) List(ctx context.Context, params pagination.PaginationParams, userID, groupID, planID *int64, status, platform, sortBy, sortOrder string) ([]service.UserSubscription, *pagination.PaginationResult, error) {
	return nil, nil, errors.New("not implemented")
}

...
import (
	...
	dbusagecleanuptask "github.com/Wei-Shaw/socialops/ent/usagecleanuptask"
...
func (r *usageCleanupRepository) ClaimNextPendingTask(ctx context.Context, staleRunningAfterSeconds int64) (*service.UsageCleanupTask, error) {
	...
	query := `
		...
		RETURNING tasks.id, tasks.status, tasks.filters, tasks.created_by, tasks.deleted_rows,
			tasks.error_message, tasks.started_at, tasks.finished_at, tasks.created_at, tasks.updated_at
	`
	var task service.UsageCleanupTask
	var filtersJSON []byte
	var errMsg sql.NullString
...
func buildUsageCleanupWhere(filters service.UsageCleanupFilters) (string, []any) {
	...
	if filters.Operation != nil {
		...
		if operation != "" {
			conditions = append(conditions, fmt.Sprintf("operation = $%d", idx))

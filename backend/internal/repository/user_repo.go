...
import (
	...
	dbuser "github.com/Wei-Shaw/socialops/ent/user"
...
func (r *userRepository) Create(ctx context.Context, userIn *service.User) error {
	...
	userIn.AllowedGroups = normalizeAllowedGroupIDs(userIn.AllowedGroups)
	return nil
}
...
func (r *userRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters service.UserListFilters) ([]service.User, *pagination.PaginationResult, error) {
	...
	groupsByUserID, err := r.loadAllowedGroups(ctx, userIDs)
	if err != nil {
		return nil, nil, err
	}
	for userID, groupIDs := range groupsByUserID {
		if u, ok := userMap[userID]; ok {
			u.AllowedGroups = groupIDs
		}
	}
...
func (r *userRepository) syncUserAllowedGroupsWithClient(ctx context.Context, client *dbent.Client, userID int64, groupIDs []int64) error {
	...
	creates := make([]*dbent.UserAllowedGroupCreate, 0, len(normalized))
	for _, groupID := range normalized {
		creates = append(creates, client.UserAllowedGroup.Create().SetUserID(userID).SetGroupID(groupID))
	}
	err := client.UserAllowedGroup.
		CreateBulk(creates...).
		OnConflictColumns(userallowedgroup.FieldUserID, userallowedgroup.FieldGroupID).
		DoNothing().
		Exec(ctx)

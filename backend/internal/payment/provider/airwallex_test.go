...
func TestAirwallexCreatePaymentUsesServerAmountAndStableRequestID(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/pa/payment_intents/create":
			...
			_, _ = w.Write([]byte(`{"id":"int_123","client_secret":"secret_123","amount":12.34,"currency":"CNY","merchant_order_id":"socialops_order","status":"REQUIRES_PAYMENT_METHOD"}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	require.Equal(t, "socialops_order", createRequests[0].MerchantOrderID)
	require.Equal(t, airwallexDeterministicRequestID("payment-intent", "socialops_order", "12.34", "CNY"), createRequests[0].RequestID)
}
...
func TestAirwallexCreatePaymentUsesConfiguredCurrency(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/pa/payment_intents/create":
			...
			_, _ = w.Write([]byte(`{"id":"int_123","client_secret":"secret_123","amount":12.34,"currency":"HKD","merchant_order_id":"socialops_order","status":"REQUIRES_PAYMENT_METHOD"}`))
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestAirwallexRequestsUseConfiguredAccountID(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/pa/payment_intents/int_123":
			paRequestCount++
			require.Equal(t, "acct_123", r.Header.Get("x-on-behalf-of"))
			_, _ = w.Write([]byte(`{"id":"int_123","amount":12.34,"currency":"CNY","merchant_order_id":"socialops_order","status":"SUCCEEDED"}`))
		...
		case "/api/v1/pa/payment_intents/int_123/cancel":
			paRequestCount++
			require.Equal(t, "acct_123", r.Header.Get("x-on-behalf-of"))
			_, _ = w.Write([]byte(`{"id":"int_123","amount":12.34,"currency":"CNY","merchant_order_id":"socialops_order","status":"CANCELLED"}`))
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestAirwallexVerifyNotificationRequiresValidSignatureAndCurrency(t *testing.T) {
	...
	raw := `{"id":"evt_1","name":"payment_intent.succeeded","accountId":"acct_123","data":{"object":{"id":"int_123","merchant_order_id":"socialops_abc","amount":88.66,"currency":"CNY","status":"SUCCEEDED"}}}`
	...
	require.Equal(t, "socialops_abc", n.OrderID)
...
func TestAirwallexQueryOrderMapsSucceeded(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/v1/authentication/login":
			_, _ = w.Write([]byte(`{"token":"token-1","expires_at":"2099-01-01T00:00:00Z"}`))
		case "/api/v1/pa/payment_intents/int_123":
			_, _ = w.Write([]byte(`{"id":"int_123","amount":99.01,"currency":"CNY","merchant_order_id":"socialops_order","status":"SUCCEEDED"}`))
		default:
			http.NotFound(w, r)
		}
	}))

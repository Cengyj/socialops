...
import (
	...
	"github.com/Wei-Shaw/socialops/internal/util/urlvalidator"
)
...
func buildTransport(opts Options) (*http.Transport, error) {
	...
	if err := configureTransportProxy(transport, parsed); err != nil {
		return nil, err
	}
...
// configureTransportProxy sets up the transport to route through the given proxy.
// Supports http/https (via http.ProxyURL) and socks5/socks5h (via x/net/proxy).

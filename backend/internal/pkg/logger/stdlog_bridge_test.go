...
func TestInferStdLogLevel(t *testing.T) {
	cases := []struct {
		...
		{msg: "[SocialTaskQueue] retry account_id=22 retry=1 max_retries=5", want: LevelInfo},
...
func TestNormalizeStdLogMessage(t *testing.T) {
	raw := "  [SocialTaskQueue]  cycle complete \n total=1   failed=0 \n"
	got := normalizeStdLogMessage(raw)
	want := "[SocialTaskQueue] cycle complete total=1 failed=0"
	if got != want {

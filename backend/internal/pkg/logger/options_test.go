...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/socialops-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/socialops-data", "logs", "socialops.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "socialops.log")

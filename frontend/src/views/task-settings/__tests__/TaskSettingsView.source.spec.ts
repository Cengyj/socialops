...
describe('TaskSettingsView source contract', () => {
  ...
  it('keeps save readiness aligned with template completeness', () => {
    ...
    expect(source).toContain("t('taskSettings.validation.postConfigurationRequired')")
    expect(source).toContain("t('taskSettings.validation.postVideoUnavailable')")
    expect(source).toContain("t('taskSettings.validation.postMediaTypeUnsupported')")
    expect(source).toContain("t('taskSettings.validation.mediaSourceUnsupported')")
  ...
  it('connects structured post, profile, avatar, and banner editors to the SocialOps-native template flow', () => {
    ...
    expect(source).toContain('socialPostMediaRefsSupported')
    expect(source).toContain('socialTaskMediaRefExecutable')
    ...
    expect(source).toContain('function normalizeMediaRef')
    expect(source).not.toContain('media_data')
    expect(zh).toContain('管理关注、点赞、转发、发帖和资料更新任务复用的执行参数模板')
    expect(zh).toContain('最多附加 4 张图片，或 1 个 MP4 视频用于发帖')
  })
  ...
  it('keeps login checks out of the task-settings page', () => {
    ...
    expect(source).toContain("const TASK_TYPES: ParameterTaskTemplateType[] = ['login', 'follow', 'like', 'retweet', 'post', 'update_profile', 'update_avatar', 'update_banner']")

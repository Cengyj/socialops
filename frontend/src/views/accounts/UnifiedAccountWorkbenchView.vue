                ...
                <div v-if="lastTaskResult" :class="['rounded-lg border px-3 py-2 text-sm', taskResultPanelToneClass]">
                  ...
                  <div v-if="taskResultPreviewLogs.length > 0" class="mt-2 space-y-1.5">
                    ...
                    >
                      ...
                      <div v-if="log.result_message" class="mt-1 line-clamp-2 opacity-80">{{ log.result_message }}</div>
                    </div>
    ...
    <BaseDialog :show="executionConfirmDialogOpen" :title="t('accountWorkbench.execution.confirmTitle')" width="wide" @close="closeExecutionConfirmDialog">
      <div class="space-y-4">
        ...
        <div class="grid gap-3 text-sm sm:grid-cols-4">
          ...
          <div class="rounded-xl border border-gray-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-800">
            <div class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ t('accountWorkbench.execution.targets') }}</div>
            <div class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ selectedTemplate?.params.targets?.length || 0 }}</div>
          </div>
          <div class="rounded-xl border border-gray-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-800">
            <div class="text-xs font-medium uppercase text-gray-500 dark:text-gray-400">{{ t('accountWorkbench.execution.contents') }}</div>
            <div class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ selectedTemplate?.params.contents?.length || 0 }}</div>
          </div>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-800">
...
const hasNonExecutableSelection = computed(() => selectedAccounts.value.some(account => !isExecutableAccount(account)))
...
function templateDisabled(template: TaskTemplate) {
  ...
  if (template.type === 'follow' || template.type === 'like' || template.type === 'retweet') {
    const targets = template.params.targets ?? []
    return !templatePoolValuesValid(targets) || targets.filter(value => value.trim()).length === 0
  }
  if (template.type === 'post') {
    const contents = template.params.contents ?? []
    return !templatePoolValuesValid(contents) || contents.filter(value => value.trim()).length === 0
  }
  return template.type !== 'login_check'
}
...
function templateSummary(template: TaskTemplate) {
  ...
  if (template.type === 'post') return t('accountWorkbench.execution.contentPoolSummary', { count: template.params.contents?.length || 0 })
  return t('accountWorkbench.execution.targetPoolSummary', { count: template.params.targets?.length || 0 })
}
...
function parseBatchImportRows(raw: string, fallbackPlatform: string): BatchImportRow[] {
  const rows = raw
    ...
    .map((item) => buildBatchImportRow(parseImportLine(item.line, fallbackPlatform), item.rowNumber))
  ...
  return rows.map((row) => {
    ...
    if (!key) return row
    if (seen.has(key)) {
      return { ...row, valid: false, status: 'batch_duplicate', error: t('accountWorkbench.import.errors.duplicateAccount') }
    }
    seen.add(key)
    return row
  })
...
function buildBatchImportRow(account: ImportSocialAccountRequest, rowNumber: number): BatchImportRow {
  const error = validateImportAccount(account)
  return { rowNumber, account, valid: !error, status: error ? 'needs_data' : 'format_valid', error }
}
...
function validateImportAccount(account: ImportSocialAccountRequest) {
  if (!trimImportValue(account.name)) return t('accountWorkbench.import.errors.accountRequired')
  if (!trimImportValue(account.password)) return t('accountWorkbench.import.errors.passwordRequired')
  ...
  if (!hasTwoFactor && !hasEmail && !hasAuthCookie) return t('accountWorkbench.import.errors.credentialRequired')
  return ''
}

        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ formatOrderAmount(row, row.refund_amount) }}</span>
...
import { adminPaymentAPI, type PaymentAuditLog } from '@/api/admin/payment'
...
async function showOrderDetail(order: PaymentOrder) {
  ...
  try {
    ...
    if (res.data.order) selectedOrder.value = res.data.order
...
function formatGatewayAmount(order: PaymentOrder, value: number): string {
  return formatPaymentAmount(value, order.currency)
}
...
function formatOrderAmount(order: PaymentOrder, value: number): string {
  ...
  return formatGatewayAmount(order, value)
}

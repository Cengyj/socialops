...
describe('task settings api', () => {
  ...
  it('saves structured post and profile-update template params through the same routes', async () => {
    const postTemplate = {
      id: 'tmpl_post',
      name: 'Rich post',
      type: 'post',
    ...
    post.mockResolvedValueOnce({ data: postTemplate })
    post.mockResolvedValueOnce({ data: profileTemplate })

    await expect(taskSettingsAPI.saveTemplate({
      name: 'Rich post',
      type: 'post',
      params: {
        contents: ['hello world'],
    ...
    await expect(taskSettingsAPI.saveTemplate({
      ...
      is_default: true,
    })).resolves.toEqual(profileTemplate)

    expect(post).toHaveBeenNthCalledWith(1, '/task-settings/templates', {
      name: 'Rich post',
      type: 'post',
      params: {
        contents: ['hello world'],
    ...
    expect(post).toHaveBeenNthCalledWith(2, '/task-settings/templates', {
      name: 'Profile refresh',
      type: 'update_profile',
      params: {
        profile: {
  ...
  it('downloads stored template media previews as blobs through the task settings media route', async () => {
    const blob = new Blob(['png-bytes'], { type: 'image/png' })
    get.mockResolvedValue({ data: blob })

    await expect(taskSettingsAPI.previewMedia('social-task/42/avatar.png')).resolves.toBe(blob)

    expect(get).toHaveBeenCalledWith('/task-settings/media', {
      params: { storage_key: 'social-task/42/avatar.png' },
      responseType: 'blob',
    })
  })
})

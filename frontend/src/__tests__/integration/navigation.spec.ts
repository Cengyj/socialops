...
const MockSubscriptions = defineComponent({
  name: 'MockSubscriptions',
  render() {
    return h('div', { class: 'subscriptions' }, 'Subscriptions')
  }
})
...
describe('Navigation Integration Tests', () => {
  ...
  describe('路由预加载', () => {
    ...
    it('路由变化时应取消之前的预加载任务', async () => {
      ...
      routePrefetch.triggerPrefetch({ path: '/subscriptions' } as any)
  ...
  describe('导航状态管理', () => {
    ...
    it('导航取消时应该正确重置状态', async () => {
      ...
      // 尝试导航到被取消的路由
      await testRouter.push('/subscriptions').catch(() => {})
      await flushPromises()

...
const collapsedActiveGroups = ref<Set<string>>(new Set())
...
function isGroupExpanded(item: NavItem): boolean {
  return expandedGroups.value.has(item.path) || (isGroupActive(item) && !collapsedActiveGroups.value.has(item.path))
}
...
function toggleGroup(item: NavItem) {
  if (isGroupExpanded(item)) {
    ...
    if (isGroupActive(item)) {
      collapsedActiveGroups.value.add(item.path)
    }
  } else {
    expandedGroups.value.add(item.path)
    collapsedActiveGroups.value.delete(item.path)
  }
}

...
const platform = computed(() => props.plan.group_platform || 'x_twitter')
...
const pLabel = computed(() => {
  ...
  return labels[platform.value] || 'Social'
})

const platformColors = computed(() => getPlatformColor(platform.value))
const borderClass = computed(() => platformColors.value.border)
const badgeLightClass = computed(() => `${platformColors.value.bg} ${platformColors.value.text}`)
const textClass = computed(() => platformColors.value.text)
const iconClass = computed(() => platformColors.value.text)
const discountClass = computed(() => `${platformColors.value.bg} ${platformColors.value.text}`)
const btnClass = computed(() => `${platformColors.value.bg} ${platformColors.value.text} hover:opacity-90`)

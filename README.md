...
- **IP and proxy management**: user-exclusive execution resources managed from the admin `/admin/proxies` workspace; accounts may keep a default execution proxy for stable executor calls, the same user's proxy may be used by that user's own accounts, account reclaim clears the default proxy, and IP/proxy tests are free.

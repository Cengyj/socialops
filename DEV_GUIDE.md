...
| 数据库凭据 | user=`socialops`, password=`socialops`, dbname=`socialops` |
...
psql -U socialops -h 127.0.0.1 -d socialops -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER socialops WITH PASSWORD 'socialops';
...
```bash
# 连接数据库
psql -U socialops -h 127.0.0.1 -d socialops
...
# 执行 SQL 文件
psql -U socialops -h 127.0.0.1 -d socialops -f migration.sql
```
...
- [上游仓库](https://github.com/Wei-Shaw/socialops)

...
SocialOps 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
>
> - **国内渠道 / 人民币结算** — [ZPay](https://z-pay.cn/?uid=23808)（`https://z-pay.cn/?uid=23808`）：支付宝 / 微信官方 API 直连，手续费 **1.6%**；资金直达商家账户，**T+1 自动到账**。支持**个人用户**（无营业执照）每日 1 万元以内交易；拥有营业执照则无限额。推荐链接可按部署需要移除。
> - **国际渠道 / USDT 或美元结算** — [启润支付](https://kyren.top/?code=SOCIALOPS)（`https://kyren.top/?code=SOCIALOPS`）：为 SaaS 产品提供低门槛国际收款通道，支持国际版微信支付与支付宝，本地货币支付、美元结算。手续费：微信 2%、支付宝 2.5%；提现 0.1%（最低 40 美元、最高 150 美元），以 **USDT 或美元**到账。无资质审核、注册即用，使用门槛最低；提现门槛略高，适合**不使用国内支付渠道、无法接受 Stripe 高达 6%+ 手续费、流水较大，且拥有美元或 USDT 渠道可接收提现资金**的用户。推荐链接可按部署需要移除。
>
...
如果你之前使用独立的外部支付服务，并在支付成功后回调 SocialOps Admin API 充值，现在可以迁移到内置支付：

### 主要差异

| 对比项 | 外部支付服务 | 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务（Next.js + PostgreSQL） | 内置于 SocialOps，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | SocialOps 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 支持 | 通过 SocialOps 订阅套餐支持 |
| 订单管理 | 独立管理界面 | 集成在 SocialOps 管理后台 |

### 迁移步骤

1. 在 SocialOps 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 SocialOps 的回调地址
3. 确认新订单通过内置支付正常处理
4. 停用外部支付服务

> **注意**：外部支付服务中的历史订单数据不会自动迁移。建议保留原服务一段时间以便查询历史记录。
